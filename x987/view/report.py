...
def theme_style(
    ...
    *,
    ...
    if bg_hex:
        parts.append(f"on {_nearest_ansi_name(bg_hex)}")
    return " ".join(parts)
        ...
        if p.exists():
            try:
                ...
                if isinstance(data, list):
                    return {
                        d["name"].strip().lower(): d["hex"]
                        for d in data
                        if "name" in d and "hex" in d
                    }
...
def get_paint_hex(ext_name: str) -> Optional[str]:
    ...
    if key in PAINT_DB:
        return PAINT_DB[key]
    if key in BUILTIN_PAINTS:
        return BUILTIN_PAINTS[key]
    try:
        ...
        if choices:
            ...
            if score >= 85:
                return PAINT_DB.get(cand) or BUILTIN_PAINTS.get(cand)
    except Exception:
        pass
    return None
...
def render_price_cell(price_value: Any, *, is_manual: bool = False) -> Text:
    ...
    if price_int is None:
        disp = (str(price_value).strip() if isinstance(price_value, str) else "").rjust(PRICE_W)
        key = "text_muted"
    else:
        k = math.ceil(price_int / 1000.0)
        disp = f"${k}k".rjust(PRICE_W)
        key = price_style_key(price_int)
    bold = key in ("teal_1", "teal_2")
    bg_key = "gray_700" if (BG_PRICE and bold) else None
    return Text(
        disp, style=theme_style(key, bold=bold, dim=(is_manual or key == "teal_6"), bg=bg_key)
    )
...
def miles_to_k_and_val(src: Any) -> tuple[Optional[int], Optional[int]]:
    ...
    if isinstance(src, str):
        s = src.strip().lower()
        m = _miles_k_pat.search(s)
        if m:
            k = math.ceil(float(m.group(1)))
            return k, k * 1000
...
def render_miles_cell(miles_value: Any, *, is_manual: bool = False) -> Text:
    ...
    if k is None:
        disp = "".rjust(MILES_W) if miles_value is None else str(miles_value).rjust(MILES_W)
        key = "text_muted"
    else:
        disp = f"{k}k".rjust(MILES_W)
        key = miles_style_key(miles_val)
    bold = key in ("teal_1", "teal_2")
...
def _model_category(model_text: str) -> str:
    s = model_text.lower()
    if "spyder" in s or re.search(r"\b(cayman|boxster)\s+r\b", s):
        return "special"
    if "cayman" in s:
        return "cayman_s" if re.search(r"\bcayman\s+s\b", s) or re.search(r"\bs\b", s) else "cayman"
    if "boxster" in s:
        return (
            "boxster_s" if re.search(r"\bboxster\s+s\b", s) or re.search(r"\bs\b", s) else "boxster"
        )
    return "other"
...
def abbreviate_transmission(tx: str) -> str:
    ...
    if m:
        g = m.group(1)
    elif re.search(r"\b(\d)\b", s):
        g = re.search(r"\b(\d)\b", s).group(1)
    if "manual" in s or re.search(r"\bm/t\b", s):
        return f"M{g}" if g else "M"
    if "pdk" in s or "dual clutch" in s or "doppelkupplung" in s:
        return "PDK"
    if "tiptronic" in s:
        return "T"
    if "dct" in s:
    ...
    if "cvt" in s:
        return "CVT"
    if "auto" in s or "at" in s or "automatic" in s:
        return f"A{g}" if g else "A"
    return (tx or "").strip().split()[0][:4].upper() or "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"
...
def render_model_cell(model_text: str, tx_text: str) -> Text:
    ...
    if not is_manual:
        key_map = {
            "cayman_s": "orange_cayman_s",
            "cayman": "orange_cayman",
            "boxster_s": "orange_boxster_s",
            "boxster": "orange_boxster",
            "special": "orange_special_1",
            "other": "orange_special_2",
        }
        key = key_map.get(cat, "orange_special_2")
        return Text(
            model_text, style=theme_style(key, bold=(cat in {"cayman_s", "boxster_s", "special"}))
        )
    dim_map = {
        "cayman_s": "warm_dim_1",
        "cayman": "warm_dim_2",
        "boxster_s": "warm_dim_3",
        "boxster": "warm_dim_4",
        "special": "warm_dim_5",
        "other": "warm_dim_6",
    }
...
def render_deal_cell(deal_value: Any, *, is_manual: bool = False) -> Text:
    ...
    return Text(
        s, style=theme_style("text_dim", dim=True) if is_manual else theme_style("text_muted")
    )


# =========================
# Colors (Ext/Int) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â two 5-char blocks
# =========================
...
# =========================
# Options ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â smart capitalization; manual rows prepend "Manual"
# =========================
_CAP_FIXES = [
    (r"\bPcm/Nav\b", "PCM/Nav"),
    (r"\bPcm\b", "PCM"),
    (r"\bPdk\b", "PDK"),
    (r"\bLsd\b", "LSD"),
    (r"\bBose\b", "BOSE"),
...
def print_table(rows: Iterable[dict]) -> None:
    ...
    for r in rows:
        deal_raw = pick(r, "Deal ÃƒÅ½Ã¢â‚¬Â ($)", "Deal", "deal_delta", default="")
    ...
    demo = [
        {
            ...
            "Price": "$24,900",
            "Miles": 34123,
            "Year/Model/Trim": "2010 Boxster S",
            "Transmission": "Automatic 7-speed (PDK)",
            "Colors (Ext / Int)": "Arctic Silver Metallic / Black",
            "Top Options": "heated seats, pcm/nav, bose",
            "Source": "cars.com",
        },
        {
            "Deal": "-1,498",
            "Price": "$43k",
            "Miles": "44k",
            "Year/Model/Trim": "2011 Cayman S",
            "Transmission": "6-speed Manual",
            "Colors (Ext / Int)": "Malachite Green Metallic / Sand Beige",
            "Top Options": "lsd, sport seats, bi-xenon",
            "Source": "cars.com",
        },
    ]
    print_table(demo)
