...
def run_rank(rows):
    ...
    for r in rs:
        ...
        parts = [str(getattr(r, "year", "") or "").strip(), str(getattr(r, "model", "") or "").strip()]
        if display_trim:
        ...
        view.append({
            ...
            "Price": (f"${text.round_up_1k(getattr(r, 'price_usd'))}" if getattr(r, "price_usd", None) else ""),
            "Miles": (text.round_up_1k(getattr(r, "mileage")) if getattr(r, "mileage", None) else ""),
            "Year/Model/Trim": ymt,
            "Transmission": getattr(r, "transmission_norm", "") or getattr(r, "transmission_raw", ""),
