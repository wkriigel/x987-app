...
def _ordered_headers(rows):
    ...
    for r in rows:
        keys.update(r.keys())
    hint = [
        "source","listing_url","vin","year","model","trim",
        "transmission_raw","transmission_norm","mileage","price_usd",
        "exterior_color","interior_color","color_ext_bucket","color_int_bucket",
        "raw_options","top5_options_count","top5_options_present","location",
        "adj_price_usd","baseline_adj_price_usd","deal_delta_usd","error",
        "timestamp_run_id",
    ]
